...
program_name = "paralela/matrixMult/bin"  # Asegúrate de que este sea el nombre correcto del ejecutable
process = subprocess.Popen(program_name, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)

# Pasar las matrices al programa en C
input_data = "\n".join(" ".join(str(val) for val in row) for row in A) + "\n" + "\n".join(" ".join(str(val) for val in row) for row in Btrans)
...
if process.returncode != 0:
    ...
    C = np.array([[float(val) for val in row.split()] for row in stdout.strip().split('\n')])
